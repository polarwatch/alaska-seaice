...
baseline_df1 = read.csv("data/bs_EasternBering.csv")
recent_df1 = read.csv("data/ext_recent_EasternBering.csv")
...
fig <- plot_ly(baseline_df1, x = ~date, y = ~upper_extent, type = 'scatter', 
      ...
      # Baseline
        add_trace(x = ~date, y = ~seaice_extent_mean, type = 'scatter', mode = 'lines',
                  line = list(color=plot_colors[3], width  = 2),name = 'Baseline', showlegend = TRUE) %>%   

      # Last year
      add_trace(data = dat_last, x = ~date, y = ~seaice_extent, type = 'scatter', mode = 'lines',
            line = list(
            color = plot_colors[2], width = 2), name = lastyear, showlegend = TRUE) %>% 

      # Current year
        add_trace(data = dat_this, x = ~date, y = ~seaice_extent, type = 'scatter', mode = 'lines',
            line = list(color=plot_colors[1], width  = 3),name = lastyear + 1, showlegend = TRUE) %>%

      # Latest point
        add_trace( x= latest$date,  y = latest$seaice_extent, type='scatter', mode = 'markers',
          marker = list(color=plot_colors[1], size=10), name = "Latest", showlegend = FALSE) 
      
      total_area = 891335.77
# Add style to chart
      fig <- fig %>% layout(title = "Daily Sea Ice Extent", hovermode = "x unified", showlegend=TRUE,
                        legend = list(
...
## Data access and method description
...
- [Daily sea ice extent baseline time series (1985-2015)](data/bs_EasternBering.csv), [metadata](data/bs_README.txt)  [.csv format]
- [Daily sea ice extent time series (current, last year)](data/ext_recent_EasternBering.csv), [metadata](data/ext_README.txt)  [.csv format]
