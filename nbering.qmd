...
baseline_df1 = read.csv("data/bs_extent_NorthernBering.csv")
recent_df1 = read.csv("data/nrt_extent_NorthernBering.csv")
...
fig <- plot_ly(baseline_df1, x = ~date, y = ~upper_extent, type = 'scatter', 
      ...
      # Last year
        add_trace(data = dat_last, x = ~date, y = ~seaice_extent, type = 'scatter', mode = 'lines',
                  line = list(color = plot_colors[2], width = 2), name = lastyear, showlegend =TRUE) %>% 
      # Current year
...
# Add style to chart
# Add style to chart
fig <- fig %>% layout(title = "Daily Sea Ice Extent", 
                      hovermode = "x unified", 
                      showlegend = TRUE,
                                              legend = list(
...
# Add style to chart
fig1 <- fig1 %>% layout(title = "Daily Sea Ice Anomalies", 
                      showlegend = TRUE,  
                      hovermode = "x unified",
                      yaxis = yaxis,
                      xaxis = xaxis,
...
- [Daily sea ice extent baseline time series (1985-2015)](data/bs_extent_NorthernBering.csv), [metadata](data/bs_extent_README.txt)  [.csv format]
- [Daily sea ice extent time series (current, last year)](data/ext_recent_NorthernBering.csv), [metadata](data/ext_README.txt)  [.csv format]
