...
This page presents monthly time series and anomalies for sea ice extent within the management regions of Alaskan waters.
...
Sea ice extent is the sum of the areas where sea ice concentration in each grid indicates sea ice. 
...
The workflow of the data processing and computation:

* CDR sea ice concentration data within the region boundaries from 1985 to the most recent annual processing are loaded

* Near-real-time sea ice concentration data within the region boundaries are appended to the CDR time series to create the data time series with the latest data available.

* Flag data are removed from the dataset.

* Sea ice concentration values are converted from fractions to binary values (0 as no ice, 1 as ice) based on the specified threshold, 0.15.

* Grid cell area value and the sea ice binary value are multiplied to obtain sea ice extent for each grid.

4. Add all sea ice extent values across all grids to obtain the sea ice extent of the region.

5. Convert the square meters to square kilometers.
